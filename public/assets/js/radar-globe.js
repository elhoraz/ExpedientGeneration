...
(function () {
    ...
    const getGlobeImg = () => getTheme() === 'light'
    ...
    world = Globe()
        ...
        .showAtmosphere(true)
        .atmosphereColor(getTheme() === 'light' ? '#a0b8cc' : '#1a2030')
    ...
    new MutationObserver(() => {
        ...
        world.atmosphereColor(getTheme() === 'light' ? '#a0b8cc' : '#1a2030');
    ...
    const openDrawer = (d) => {
        document.getElementById('idAvatar').src = d.foto ? '/uploads/profiles/' + d.foto : '/images/default-avatar.jpg';
    ...
    searchInput.addEventListener('input', () => {
        ...
        searchResults.innerHTML = matches.map(m =>
            `<div class="sr-item" data-id="${m.id}">
                <img class="sr-avatar" src="${m.foto ? '/uploads/profiles/' + m.foto : '/images/default-avatar.jpg'}" alt="">
            ...
            pos => {
                ...
                fetch(`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${lat}&longitude=${lng}&localityLanguage=id`)
                    ...
                    .then(data => {
                        ...
                        if (data.status === 'success') { showStatus('Berhasil disimpan!', '#4ade80'); setTimeout(() => location.reload(), 1500); }
