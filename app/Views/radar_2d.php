...
<?= $this->section('title') ?>Jaringan Silaturahmi (Peta 2D)<?= $this->endSection() ?>

<?= $this->section('styles') ?>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.min.css" />
<style>
/* ===== MAP CONTAINER ===== */
#mapViz { position:fixed; inset:0; width:100vw; height:100vh; z-index:2; background:#1a1c23; }
.main-wrapper { overflow:hidden !important; }

/* Menghilangkan border focus biru bawaan Leaflet */
.leaflet-container:focus { outline: none; }
/* Custom gaya untuk Leaflet popup agar sesuai tema Glassmorphism */
.leaflet-popup-content-wrapper, .leaflet-popup-tip {
    background:rgba(5,10,8,.92)!important; 
    backdrop-filter:blur(20px)!important; 
    border:1px solid rgba(212,175,55,.3)!important; 
    color: #fff;
}
.leaflet-popup-content-wrapper { border-radius:14px!important; padding:4px!important; }
...
.stat-box { background:var(--glass-bg); backdrop-filter:var(--glass-blur); border:1px solid var(--glass-border); border-left:3px solid #d4af37; padding:8px 16px; border-radius:10px; box-shadow:var(--glass-shadow); pointer-events:auto; }
...
@media(max-width:768px) {
    ...
    .radar-hud { top:75px; left:20px; right:20px; display:flex; justify-content:space-between; align-items:center; background:var(--glass-bg); backdrop-filter:var(--glass-blur); border:1px solid var(--glass-border); padding:10px 15px; border-radius:15px; pointer-events:none; }
...
<!-- MAP CONTAINER (Replaces globeViz) -->
<div id="mapViz"></div>
...
<script>
    // Fallback: hapus loading jika terjadi error javascript agar peta/UI tetap terlihat atau minimal loading hilang
    window.addEventListener('error', function(e) {
        var rl = document.getElementById('radarLoading');
        if (rl) {
            rl.style.opacity = '0';
            setTimeout(() => rl.remove(), 500);
            console.error("Radar 2D Error:", e.message);
        }
    });

    window.__radarData = <?= json_encode($alumni_nodes) ?>;
    window.__mapStyle = '<?= isset($mapStyle) ? $mapStyle : 'classic' ?>';
